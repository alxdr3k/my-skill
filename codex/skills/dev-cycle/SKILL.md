...
사이클 시작 전 `git remote get-url origin` 또는 repo root 이름으로 리포 이름을
확인하고, 이후 모든 브랜치/ship 판단에 같은 값을 사용한다.
...
리포 이름이 위 목록에 있으면 Direct-push 리포로 취급한다. Direct-push 리포에서는
작업 브랜치와 PR을 만들지 않고 `main`에서 직접 작업한 뒤 `main`으로 push한다.

그 외 모든 리포는 Standard 리포로 취급한다. Standard 리포는 작업 브랜치에서
구현하고 PR을 만든다.

## Branch / PR 원칙

기본은 PR-first workflow다. 단, Direct-push 리포는 위 "리포지토리 유형 정의"가
우선하며 PR-first 원칙을 적용하지 않는다.

- Direct-push 리포에서는 `main`에서 직접 작업하고 PR을 만들지 않는다.
- Standard 리포의 `main` / `master` / default branch에서는 작업 브랜치를 만든다.
- Standard 리포에서 이미 작업 브랜치에 있으면 현재 브랜치를 유지한다.
- Standard 리포의 direct push는 사용자가 명시적으로 요청한 경우에만 한다.
...
- Direct-push 리포는 새 브랜치를 만들지 않는다. `main`에서 직접 작업한다.
- Direct-push 리포인데 현재 브랜치가 `main`이 아니면 `main`으로 전환하고,
  fast-forward가 가능한 경우에만 최신 `origin/main`으로 맞춘다.
- Standard 리포는 default branch에서 시작했다면 위 명명 규칙으로 작업 브랜치를
  만든다. 이미 작업 브랜치에 있으면 현재 브랜치를 유지한다.
...
`dev-cycle` 실행 자체는 ship까지 포함한 요청으로 취급한다. 단, 사용자가
`push하지 마`, `PR 만들지 마`, `ship 제외`처럼 publish 금지를 명시한 경우에는
Step 8에서 멈추고 로컬 변경 상태만 보고한다.

Direct-push 리포:

1. 현재 브랜치가 `main`인지 확인한다. 아니면 멈추고 원인을 보고한다.
2. `git status -sb`와 staged diff를 확인한다.
3. 의도한 파일만 stage한다.
4. commit한다.
