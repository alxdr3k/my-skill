...
현재 브랜치에 다른 base를 섞지 않는다. 특히 feature branch에서 `git pull origin main`
을 실행하지 않는다.
...
**Direct-push 리포:** `main`만 fast-forward한다.
...
**Standard 리포:** `origin/dev`가 있으면 dev만 fast-forward한다. 현재 브랜치가
feature branch이면 그 브랜치에 `main` 또는 `dev`를 pull하지 않는다.
...
if git show-ref --verify --quiet refs/remotes/origin/dev; then
  if [ "$CURRENT_BRANCH" = "dev" ]; then
    ...
    echo "Dirty worktree: fetched origin/dev, skipped local dev checkout"
  fi
fi
```
