...
- `--phase <id>` : 지정한 Phase 완료 후 즉시 중단. id 형식은 자유롭다 (`1`, `3`, `1B`, `2-ㄱ`, `4-dragonfly123` 등).
...
**`--phase` 파싱 규칙:**
`<id>`에서 맨 앞의 연속된 숫자를 추출해 대상 Phase 번호로 사용한다.
숫자 뒤의 문자열(접미사)은 무시한다.
...
각 Phase 완료 시점에 현재 번호와 추출된 번호를 비교해 일치하면 중단하고 사용자에게 보고한다.
