...
argument-hint: [--loop [N]] [--phase <project-phase-id>] [--until <step>]
---
...
- `--phase <id>` : **구현 대상 프로젝트 Phase 지정.** Phase 4(구현)에서 이 id에 해당하는 작업만 수행한다. 형식 자유 (`1B`, `1A.8`, `3`, `2-ㄱ` 등 프로젝트 문서의 Phase 식별자 그대로 사용).
- `--until <N>` : dev-cycle 워크플로우 자체를 N단계 완료 후 중단 (N = 1~7).

`$ARGUMENTS` 파싱:
- `--phase <id>` 는 **프로젝트 로드맵의 Phase id** 이다. dev-cycle 워크플로우 단계 번호와 무관하다.
- `--until <N>` 만이 워크플로우 중단을 제어한다.
- 두 플래그는 독립적으로 동작한다. 예: `--phase 1B --until 5` = 1B를 구현 대상으로 Phase 5(Verify)까지만 실행.
...
> `--until 6` 이면 리뷰 pass 후 중단 (Phase 7 실행하지 않음).
